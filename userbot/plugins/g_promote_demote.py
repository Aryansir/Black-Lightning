#Credit --> Danish
# part of lethal projects
#Modified by --> @hacker11000
marculs=9
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        if user.isnumeric():
            user = int(user)
        if not user:
            await event.edit("`Put a valid id.`")
            return
        ...
        try:
            ...
            return await event.edit("Sorry, An error occured contact @lightning_support_group", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@borg.on(lightning_cmd(pattern="gpromote ?(.*)"))
async def gben(userbot):
    bl = lightning = userbot
    i = 0
    sender = await bl.get_sender()
    me = await userbot.client.get_me()
    await dark.edit("`Wait global promoting started by BL..`")
    ...
    try:
        ...
        pass
    if me == user:
       k = await dark.edit("master don't put your id...")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    except:
        return await dark.edit(f"**An error occured...**")
    if user:
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
       k = await dark.edit("Put a valid id ....")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
