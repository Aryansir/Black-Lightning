...
async def pm(client, message):
    ...
    if his_turn(mister.id) == PM_LIMIT and not get_user(mister.id) == mister.id:
        await app.send_message(message.chat.id, "__I warned you 5 times now its time for action byy__\n\n**BLOCKED**")
    ...
    bot_results = await client.get_inline_bot_results(f"{g}", "**Black")
...
async def ap(client, message):
    try:
       if hd_no(message.text):
         ...
         ok= await app.get_users(int(message.chat.id))
